import React, { Component } from 'react'
import { BrowserRouter, Route, Link } from 'react-router-dom'

class Navbar extends Component {
    render() {
        return (
            <div>
                <header>
                    <div className="row">
                        <div className="col-md-1">
                            <img src="data:image/png;base64," alt="" />
                        </div>

                        <div className="col-md-4">
                            <div className="pages">
                                <ul >
                                    <li >
                                        <Link exact to="/" activeClassName="active" className="alink">Store </Link>
                                    </li>
                                    <li>
                                        <Link to="/news" activeClassName="active" className="alink">News </Link>
                                    </li>
                                    <li >
                                        <Link to="/faq" activeClassName="active" className="alink">Faq </Link>
                                    </li>
                                    <li >
                                        <Link to="/help" activeClassName="active" className="alink">Help </Link>
                                    </li>
                                    <li>
                                        <Link to="/unrealengine" activeClassName="active" className="alink">Unreal Engine </Link>
                                    </li>
                                </ul>
                            </div>
                        </div>

                        <div className="col-md-7">
                            <div className="navright">
                        <Link ><i class="fas fa-globe"  onMouseOver="langsover()"></i></Link>
                        <Link><i class="fas fa-user-check"></i></Link>
                            <Link className="giris">GİRİŞ YAP</Link>
                            <button>EPIC GAMES'I INDIR</button>
                        </div></div>
                    </div>
                </header>
                <div className="langs" id="langs">
                <ul>
                        <li>العربية</li>
                        <li>Deutsch</li>
                        <li>Español (Spain)</li>
                        <li>Español (LA)</li>
                        <li>Français</li>
                        <li>Italiano</li>
                        <li>日本語</li>
                        <li>한국어</li>
                        <li>Polski</li>
                        <li>Português (Brasil)</li>
                        <li>ไทย</li>
                        <li>Türkçe</li>
                        <li>简体中文</li>
                        <li>繁體中文</li>

                    </ul>
                </div>
            </div>
        )
    }
}

export default Navbar


